...
#pragma warning restore RCS1090 // Call 'ConfigureAwait(false)'.
        ...
        private string ResultsIndex(HttpContext context)
        {
            ...
            return Render.ResultListHtml(Options, path, context.Request.QueryString.HasValue ? context.Request.QueryString.Value : string.Empty);
        }
        ...
        private string ResultsStatIndex(HttpContext context)
        {
            ...
            var path = context.Request.PathBase + Options.RouteBasePath.Value.EnsureTrailingSlash();
            return Render.ResultStatListHtml(Options, path, context.Request.QueryString.HasValue ? context.Request.QueryString.Value : string.Empty);
        }
        ...
        private async Task<string> ResultsListAsync(HttpContext context)
        {
            ...
            var top = 100;
            if (!string.IsNullOrWhiteSpace(take) && int.TryParse(take, out int t) && t > 0)
            {
                top = t;
            }
