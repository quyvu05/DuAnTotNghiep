...
namespace Shop.Module.Orders.Services
{
    public class OrderService : IOrderService
    {
		...
		public async Task<OrderCreateResult> OrderCrateByCart2(int cartId, OrderCreateBaseParam param, string adminNote = null)
		{
			...
			{
				throw new Exception("The quantity of the product must be greater than 0");
			}
		...
		public async Task<Order> OrderCreate(int userId, OrderCreateBaseParam param, string adminNote = null)
		{
			...
			if (param.Items == null || param.Items.Count <= 0)
				throw new Exception("The purchased item does not exist or has been removed from the shelves");
			...
			foreach (var item in param.Items)
			{
				...
				OrderStockDoWorker(stocks, addStockHistories, product, userId, -item.Quantity, order, "Create an order");
            ...
            using (var transaction = _orderRepository.BeginTransaction())
            {
				...
				await _jobService.Schedule(() => Cancel(order.Id, (int)UserWithId.System, "Timeout automatic cancellation"), TimeSpan.FromMinutes(min));
		...
		public async Task<PaymentOrderBaseResponse> PayInfo(int orderId)
		{
			...
			{
				throw new Exception("Current order status does not allow payment");
			}

			var userLogin = await _userLoginRepository.Query().FirstOrDefaultAsync(c => c.UserId == user.Id); 
			if (string.IsNullOrWhiteSpace(userLogin?.ProviderKey))
			{
				throw new Exception("Buyer information exception");
			}
		...
		void OrderStockDoWorker(IList<Stock> stocks, IList<StockHistory> addStockHistories, Product product, int userId, int quantity, Order order, string note)
		{
			...
			if (stocks.Count <= 0)
				throw new Exception("Product stock does not exist");
		...
		public async Task<OrderAddressResult> GetOrderAddress(int orderAddressId)
		{
			...
			if (provinces == null || provinces.Count <= 0)
				throw new Exception("Province data exception, please contact the administrator");
        ...
        public async Task PaymentReceived(PaymentReceivedParam param)
        {
			...
			foreach (var item in order.OrderItems)
			{
				OrderStockDoWorker(stocks, addStockHistories, item.Product, userId, -item.Quantity, order, "Mark as paid");
			}
