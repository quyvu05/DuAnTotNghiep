...
namespace Shop.Module.Catalog.Controllers
{
	...
	public class ProductApiController : ControllerBase
	{
		...
		public async Task<Result> Delete(int id)
		{
			...
			//If an order has been generated for a product, product deletion is not allowed temporarily
			var anyOrder = await _orderItemRepository.Query().AnyAsync(c => c.ProductId == product.Id);
			if (anyOrder)
			{
				return Result.Fail("The current product has been ordered, and deletion is not allowed temporarily");
			}
		...
		/// <summary>
		/// Publish the product with the specified ID and make it visible on the frontend.
		/// </summary>
		/// <param name="id">Product ID. </param>
		/// <returns>Return the operation result. </returns>
		...
		public async Task<Result> Publish(int id)
		{
			...
			{
				return Result.Fail("The parent product corresponding to the current product is not published, operation not allowed");
			}
		...
		private void UpdateAndDeleteStock(IEnumerable<Stock> productStocks, List<StockHistory> addStockHistories, Product product, User user, IEnumerable<ProductCreateStockParam> stockParams)
		{
			...
			{
				if (item.Quantity < 0)
					throw new Exception("Stock quantity must be >= 0");
